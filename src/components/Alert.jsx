...
export default function Alert({ show, close, message, type }) {
  const alertTypes = {
    info: {
  ...
  return (
    <div className="fixed right-0 top-[38px] h-0 w-fit z-40" role="alert">
      <div
        className={`w-fit h-fit rounded-lg text-sm p-4 ${alertTypes[type] ? alertTypes[type].styles : ""} ${
          show ? "flex" : "hidden"
        }`}
      >
        <svg
        ...
        <button
          ...
          className={cx(
            ...
            {
              [`${alertTypes[type] ? alertTypes[type].styles : ""}`]: alertTypes[type],
            },
          )}
          aria-label="Close"
          onClick={() => close()}
