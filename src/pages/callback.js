...
const App = ({ location }) => {
  ...
  return (
    <Layout noChanges={() => { return true }} path={location.pathname} showNavBar={false} showFooter={false}>
      <div className="h-screen bg-gradient-radial from-zinc-100 from-60% to-zinc-400 relative">
        <div className="flex flex-row justify-center items-center h-full px-4 sm:w-9/12 m-auto">
          {loginInProgress ? <h3 className="text-xl text-black">
            <LoadingIndicator /> Logging in...
          </h3>
            : loggedIn ? <div className="flex flex-inline flex-col items-center">
              <h3 className="text-xl text-black"><LoadingIndicator /> Login successful! Redirecting...</h3>
              <ActionButton customStyling="bg-black text-white hover:bg-black hover:text-gray-200" onClick={() => { toPrevPage() }} text="Click here if not redirected" />
            </div>
              : error ? <div>
                ...
                <div className="mt-2">
                  ...
                  <LoginButton cleanupFunc={() => { if (loggedIn) { navigate("/my-brackets") } }} />
                </div>
              </div>
                : <div>
                  <span className="font-bold text-lg">Error logging in!</span>
